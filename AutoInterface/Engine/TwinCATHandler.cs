        ...
        public void EnableCoresForRT(int cores)
        {
            ...
            using (StringWriter stringWriter = new StringWriter())
            {
                using (XmlWriter writer = XmlTextWriter.Create(stringWriter))
                {
                    ...
                    {
                        case 1:
                            affinityString = string.Format("#x{0}", ((ulong)CpuAffinity.MaskSingle).ToString("x16"));
                            break;
                        case 2:
                            affinityString = string.Format("#x{0}", ((ulong)CpuAffinity.MaskDual).ToString("x16"));
                            break;
                        case 4:
                            affinityString = string.Format("#x{0}", ((ulong)CpuAffinity.MaskQuad).ToString("x16"));
                            break;
                        default:
                            affinityString = string.Format("#x{0}", ((ulong)CpuAffinity.MaskSingle).ToString("x16"));
                            break;
